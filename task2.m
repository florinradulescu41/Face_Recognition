function task2()
  ...
  vs1 = sort(vs1, 'descend');
  vs2 = sort(vs2, 'descend');

  subplot(4, 2, 1);
  plot(vs1);
  title("task 2, image3.gif, grafic 1");
  subplot(4, 2, 2);
  plot(vs2);
  title("task 2, image4.gif, grafic 1");  
  ...
  subplot(4, 2, 3);
  plot(k, info1);
  title("task 2, image3.gif, grafic 2");
  ...
  subplot(4, 2, 4);
  plot(k, info2);
  title("task 2, image4.gif, grafic 2");
  ...
  for t=1:length(k)
    A_k = task1('in/images/image3.gif', k(t));
    sum3_1 = 0;
    for i=1:m1
      for j=1:n1
  ...
  subplot(4, 2, 5);
  plot(k, err1);
  title("task 2, image3.gif, grafic 3");
  
  k=[1:19 20:20:99 100:30:min(m2, n2)];
  err2 = zeros(length(k), 1);
  for t=1:length(k)
    B_k = task1('in/images/image4.gif', k(t));
    sum3_2 = 0;
    for i=1:m2
      for j=1:n2
  ...
  subplot(4, 2, 6);
  plot(k, err2);
  title("task 2, image4.gif, grafic 3");
  ...
  for i=1:length(k)
    rcd1(i) = (m1*k(i) + n1*k(i) + k(i))/(m1*n1);
  endfor

  subplot(4, 2, 7);
  plot(k, rcd1);
  title("task 2, image3.gif, grafic 4");
  
  k=[1:19 20:20:99 100:30:min(m2, n2)];
  rcd2 = zeros(length(k), 1);
  for i=1:length(k)
    rcd2(i) = (m2*k(i) + n2*k(i) + k(i))/(m2*n2);
  endfor

  subplot(4, 2, 8);
  plot(k, rcd2);
  title("task 2, image4.gif, grafic 4");
